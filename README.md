...
- **CP3 — refinamientos (funcionando):** modelo de marcador **Poisson bivariada + Dixon-Coles** opcional (`model.match_model: dixon_coles` en `config.yaml`): los goles esperados de cada equipo salen de su fuerza efectiva (escala Elo) y los marcadores de dos Poisson independientes con la corrección de Dixon-Coles para los resultados bajos — marcadores más realistas, mejor fidelidad al desempate por diferencia de goles. `descenso calibrate` autocalibra `alpha` / `form_half_life_days` / `form_k_factor` minimizando el Brier del backtest (scipy Nelder-Mead, seed fija; no toca `config.yaml`, solo sugiere). `descenso report --html informe.html` escribe un informe HTML estático autocontenido (CSS y gráfico de barras SVG en línea, sin JavaScript). Tras el CP0: `descenso history` muestra cómo ha evolucionado la P(descenso) de cada equipo jornada a jornada esta temporada, y `simulate`/`report`/HTML llevan una nota fija recordando que el modelo **sí** simula el calendario restante (la duda nº1 de la afición). La feature de **sentimiento (NLP)** se descartó: el CP0 (ver [`docs/community-factors.md`](docs/community-factors.md)) no encontró demanda real para ella.
...
descenso history --gameweeks 8              # cómo ha evolucionado la P(descenso) jornada a jornada esta temporada
